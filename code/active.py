...
# # TASK 1 ONLY!!! 
# def CNN_score(filename, X_w, X_i, y, word2ind, maxWords, ind2label, maxChar, char2ind, pretrained_embedding="", word_embedding_size=100,
              # char_embedding_size=50, lstm_hidden=32, dropout=0.5, optimizer='rmsprop',
              # folder_path="CNN_results", score_name = "scores",
              # train = False, X_train = None, y_train = None, X_test = None, y_test = None,
              # nbr_epochs = 0, batch_size=128, early_stopping_patience=-1):   
    # """
        # TODO: function and parameters description
    # """
...
# TASK 1 ONLY!!! 
# def CNN_query(filename, X_w, X_i, y, word2ind, maxWords, ind2label, maxChar, char2ind, numSeqToQuery, seed = 42, verbose = False, 
              # write_to_disk = False, pretrained_embedding="", word_embedding_size=100,
              # char_embedding_size=50, lstm_hidden=32, dropout=0.5, mode = "least", optimizer='rmsprop',
    ...
    # # Full word representation                             
    # word_full = concatenate([word_emb, char_enc])
    ...
    # # TODO: DEBUG OUTPUT 
    # query_index_least_rank = sequence_entropy_sort_index[:numSeqToQuery]
    # query_index_last_rank = sequence_entropy_sort_index[max((len(sequence_entropy_sort_index) - numSeqToQuery),0):]
    # query_index_random = np.arange(1, len(sequence_entropy_sort_index) + 1)
    # np.random.seed(seed)
    # np.random.shuffle(query_index_random)
    # query_index_random = query_index_random[:numSeqToQuery]
    ...
    # print("Least")
    # print(query_index_least_rank.shape, sequence_entropy.shape)
    # print(query_index_least_rank)
    # print(np.concatenate((np.array([0]), sequence_entropy), axis = -1)[query_index_least_rank])
    # print("Most")
    # print(query_index_last_rank.shape, sequence_entropy.shape)
    # print(query_index_last_rank)
    # print(np.concatenate((np.array([0]), sequence_entropy), axis = -1)[query_index_last_rank])
    # print("Rand")
    # print(query_index_random.shape, sequence_entropy.shape)
    # print(query_index_random)
    # print(np.concatenate((np.array([0]), sequence_entropy), axis = -1)[query_index_random])
    
    # # TODO: add writing support for task2 and task3
    # # Store least index rank
    # if mode == "least":
        ...
        # if write_to_disk:
            ...
            # with open(query_result_path, "w", encoding = "utf-8") as f:
                ...
                # else:
                    # f.write("-DOCSTART- -X- -X- o\r\r")
                # # Store least index rank
                # for i in query_index_least_rank: 
                    # seq = X_w[i]
                    # seq_offset = i*maxWords + seq_len - len(seq)
                    # for j,w in enumerate(seq):
                        # index = seq_offset + j
    ...
    # if mode == "most":
        ...
        # if write_to_disk:
            ...
            # with open(query_result_path, "w", encoding = "utf-8") as f:
                # if verbose:
                    # f.write("token target predicted entropy\n\n")
                # else:
                    # f.write("-DOCSTART- -X- -X- o\r\r")
                # for i in query_index_most_rank: 
                    # seq = X_w[i]
                    # seq_offset = i*maxWords + seq_len - len(seq)
                    # for j,w in enumerate(seq):
                        # index = seq_offset + j
    ...
    # if mode == "hybrid":
        ...
        # query_index_least_rank = sequence_entropy_sort_index[:int(numSeqToQuery*(1/3))]
        # query_index_most_rank = sequence_entropy_sort_index[max((len(sequence_entropy_sort_index) - int(numSeqToQuery*(2/3))),0):]
